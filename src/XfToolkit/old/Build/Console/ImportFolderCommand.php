...
class ImportFolderCommand extends Command {
	...
	protected function copyLibrary($libraryPath)
	{
		...
		$this->info('Linking library files');
		$parts = array_reverse(explode('/', realpath($libraryPath)));
		$this->symlink(realpath($libraryPath), $this->application->getXfLibPath().$parts[1].'/'.$parts[0]);
	}
	...
	protected function copyDependencies($directory)
	{
		...
		$this->info('Linking dependencies into library');
		foreach ($libs AS $namespace => $lib)
		{
			$namespace = str_replace('\\', '/', $namespace);
			$this->symlink($lib.'/'.$namespace, $this->application->getXfLibPath().$namespace);
		}
	}
	...
	protected function symlink($from, $to)
	{
		$this->line($from.' => '.$to);

		if (file_exists($to))
		{
			$this->line("Warning: '$to' exists");
			return;
		}

		if ( ! file_exists(dirname($to)))
		{
			$this->fileSystem->makeDirectory(dirname($to));
		}
	...
	protected function getOptions()
	{
		return array(
			...
			array('hardcopy', null, InputOption::VALUE_NONE, 'Hard copy instead of symlink, windows will only hardlink', null),
